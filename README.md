...
This project uses one second EEG segments to classify seizure vs non seizure brain activity, comparing a simple interpretable baseline against a stronger ensemble model, and evaluating both with metrics appropriate for a highly imbalanced classification task.
...
Balancing helped logistic regression substantially (F1: 0.12 → 0.38) by forcing it to actually predict the minority class more often, though it remains a weak model overall. The linear decision boundary is a poor fit for this data. The random forest, which was already well calibrated for the imbalance on its own, saw a slight *decrease* in seizure F1 (0.94 → 0.93). 
...
- A system that only reacts internally (e.g. triggering stimulation) can tolerate more false positives favours a lower threshold.
- A system that raises clinician alerts needs to avoid alert fatigue favours a higher threshold.
...
- This is single second, non sequential classification. A real device would need to process a continuous EEG stream.
